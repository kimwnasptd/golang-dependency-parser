...
tail -n +2 "$CSV_FILE" | while IFS=',' read -r Rock Language Repo CommitTag ProjectDir Comments; do
  ...
  ProjectDir=$(echo "$ProjectDir" | xargs)
  ...
  # If ProjectDir is empty, use an empty string, which will result in just 'scripts/'
  # being appended to the repo path.
  TARGET_SUB_DIR=""
  if [ -n "$ProjectDir" ]; then
    TARGET_SUB_DIR="$ProjectDir"
    echo "  'project dir' specified. Using subdirectory: '$TARGET_SUB_DIR'."
  else
    TARGET_SUB_DIR="" # Empty string means the repo root itself for path construction
    echo "  'project dir' not specified. Defaulting to repository root for 'parse' target."
  fi
